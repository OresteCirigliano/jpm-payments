...
def generate(df, payment_date, month_abbrev):
    ...
    df_c['DepositRoutingNumber'] = df_c['DepositRoutingNumber'].astype(str).str.replace('-', '', regex=False).str.strip()
    ...
    for _, rec in df_g.iterrows():
        ...
        tr = ['TR', pid, payment_date, CFG['country_code'], '',
              str(rec['routing_number']).strip(), str(rec['account_number']).strip(),
    ...
    return buf, num_tr, total_pence / 100, 'GBP', gen_ids, gen_totals
