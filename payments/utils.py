...
def extract_generated_data(df_grouped, amount_col='total_amount'):
    """Estrae ids e totali dal DataFrame raggruppato per la validazione."""
    generated_ids    = set(df_grouped['partner_id'].astype(str).str.strip())
    generated_totals = dict(zip(
        df_grouped['partner_id'].astype(str).str.strip(),
        df_grouped[amount_col].round(2)
    ))
    return generated_ids, generated_totals
