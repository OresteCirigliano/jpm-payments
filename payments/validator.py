...
def validate(df_emea, country_code, emea_filter_code, generated_ids, generated_totals, sodexo_exclude=False):
    ...
    # 2. Logica di Validazione e Confronto
    for cid in all_emea_ids:
        ...
        # Logica Terze Parti (Sodexo/PayQuicker)
        has_sodexo = False
        ...
        has_bank = (iban.upper() not in ('NULL', 'NAN', '') or 
        ...
        if cid in generated_ids:
            ...
            if abs(diff) > 0.01:
                anomalies.append({
                    'CustomerID': cid, 'Type': 'Amount discrepancy',
                    'EMEA Amount': total_emea, 'Generated Amount': total_gen,
                    'Difference': diff, 'Detail': f'Expected {total_emea}, gen {total_gen}'
                })
        else:
            # --- GERARCHIA DELLE ESCLUSIONI (PRIORITÀ AGGIORNATA) ---
            if total_emea <= 0:
                ...
                anomalies.append({
                    'CustomerID': cid, 'Type': 'Excluded without clear reason',
                    'EMEA Amount': total_emea, 'Generated Amount': 0,
                    'Difference': -total_emea, 'Detail': 'Present in EMEA but not in file'
                })

    # Record presenti nel file ma non in EMEA
    for cid in set(generated_ids) - all_emea_ids:
        anomalies.append({
            'CustomerID': cid, 'Type': 'ID not found in EMEA',
            'EMEA Amount': 0, 'Generated Amount': generated_totals.get(cid, 0),
            'Difference': generated_totals.get(cid, 0), 'Detail': 'ID missing from EMEA source'
        })
